...
var _webImmediateJs = require("core-js/modules/web.immediate.js"); // console.log);
var _runtime = require("regenerator-runtime/runtime");
var _modelJs = require("./model.js");
var _helperJs = require("./helper.js");
var _taskBoxViewJs = require("./views/taskBoxView.js");
var _taskBoxViewJsDefault = parcelHelpers.interopDefault(_taskBoxViewJs);
const controlTask = function(taskData) {
    (0, _taskBoxViewJsDefault.default).toggleWindow();
    _modelJs.pushTask(taskData);
    (0, _taskBoxViewJsDefault.default).displayTaskes(_modelJs.taskObj());
};
// time => time stamp that comes from btn close from task box view
const controlRemoveTask = function(timeStamp) {
    _modelJs.removeCompletedTask(timeStamp);
};
const controlRecevedTaskes = function() {
    return _modelJs.state.taskes;
};
const init = function() {
    (0, _taskBoxViewJsDefault.default).addHandelercreateTask(controlTask);
    (0, _taskBoxViewJsDefault.default).addHandelerRemoveTask(controlRemoveTask);
    (0, _taskBoxViewJsDefault.default).renderRecevedTaskes(controlRecevedTaskes());
    (0, _taskBoxViewJsDefault.default).addHandelerSelectBox();
};
init();

},{"core-js/modules/web.immediate.js":"49tUX","regenerator-runtime/runtime":"dXNgZ","./views/taskBoxView.js":"hOPch","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","./model.js":"Y4A21","./helper.js":"lVRAz"}],"49tUX":[function(require,module,exports) {
"use strict";
...
},{}],"hOPch":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _mainFnJs = require("./mainFn.js");
var _mainFnJsDefault = parcelHelpers.interopDefault(_mainFnJs);
var _helperJs = require("../helper.js");
class TaskBox extends (0, _mainFnJsDefault.default) {
    _parentElement = document.querySelector(".upload__task");
    _taskesBox = document.querySelector(".taskes__box");
    _btnOpen = document.querySelector(".btn__add--task");
    _btnClose = document.querySelector(".btn--close-modal");
    _overlay = document.querySelector(".overlay");
    _taskWindow = document.querySelector(".add-task-window");
    _btnRemoveTask = document.querySelector(".content__task");
    _selectBox = document.querySelector(".select__box");
    #createdTaskes = [];
    #completedTaskes = [];
    #data;
    constructor(){
    ...
    toggleWindow() {
        ...
        this._taskWindow.classList.toggle("hidden");
    }
    ...
    addHandelercreateTask(handeler) {
        // this._taskesBox.innerHTML = '';
        this._parentElement.addEventListener("submit", function(e) {
            e.preventDefault();
            const dataArr = [
                ...new FormData(this)
            ]; //[[key, value]]
            const data = Object.fromEntries(dataArr); //{key:value}
            const taskTitle = document.querySelector(".input__task--name");
            const taskDescription = document.querySelector(".input__task--description");
    ...
    randomIcon() {
        ...
        return icons[(0, _helperJs.randomCorrectNum)(0, icons.length - 1)];
    }
    displayTaskes(obj) {
        this.#createdTaskes.push(obj);
        this.generateMarkup(obj);
    }
    generateMarkup(obj) {
        obj.randomIcon = this.randomIcon();
        this.#data = obj;
  ...
  <div class="content__task">
    ...
    <h3 class="task__title">${this.#data.taskTitle} <span class="random__icons">${obj.randomIcon}</span> </h3>
    <p class="date">${this.#data.date}
  ...
  <button class="close"
  data-time-stamp = ${this.#data.timeStamp}
  >X</button>
  </div>
  </div>
  </div>
    </li> `;
        this._taskesBox.insertAdjacentHTML("afterbegin", markup);
    }
    addHandelerRemoveTask(handeler) {
        let completedTask = {};
        this._taskesBox.addEventListener("click", (e)=>{
            completedTask = {};
            const btn = e.target.closest(".close");
            if (!btn) return;
            const timeStamp = +btn.dataset.timeStamp;
            const taskRow = e.target.closest(".content__task");
            if (!taskRow) return;
            const allEleTaskBox = taskRow.querySelectorAll("*");
            Array.from(allEleTaskBox, (e)=>{
                if (e.getAttribute("class") === "circle__logo") {
                    e.style.fontSize = "2rem";
                    e.textContent = "\u2714\uFE0F";
                    e.style.color = "white";
                }
                if (e.getAttribute("class") === "task__title") {
                    completedTask.taskTitle = e.textContent;
                    e.classList.add("completed");
                }
                if (e.getAttribute("class") === "description") e.innerHTML = "";
                if (e.getAttribute("class") === "date") e.innerHTML = "";
                if (e.getAttribute("class") === "time") e.innerHTML = "";
            });
            handeler(timeStamp);
            completedTask.created = false;
            this.#completedTaskes.push(completedTask);
            const index = this.#createdTaskes.findIndex((task)=>{
                task.timeStamp;
            });
            this.#createdTaskes.splice(index, 1);
        });
    }
    renderRecevedTaskes(arr) {
        this.#data = arr;
        this.#data.forEach((task)=>this.#createdTaskes.push(task));
        window.addEventListener("load", ()=>{
            this.#data.forEach((task)=>{
                this.generateMarkup(task);
            });
        });
    }
    addHandelerSelectBox() {
        this._selectBox.addEventListener("change", (e)=>{
            if (!this.#completedTaskes.length > 0) {
                alert("you are not completed task");
                e.target.value = "All";
            }
            this._taskesBox.innerHTML = "";
            if (e.target.value === "Taskes removed") this.displayRemovedTask(this.#completedTaskes);
            else {
                this.displayRemovedTask(this.#completedTaskes);
                this.displayCompletedTask(this.#createdTaskes);
            }
        });
    }
    displayRemovedTask(obj) {
        obj.forEach((task)=>this.generateMarkupCompletedTask(task));
    }
    displayCompletedTask(obj) {
        obj.forEach((task)=>this.generateMarkup(task));
    }
    generateMarkupCompletedTask(obj) {
        const html = `
  <li class="task__row">
    <div class="content__task">
      <div class="text">
      <div class="circle__logo" style="font-size: 2rem; color: white;">\u{2714}\u{FE0F}</div>
      <h3 class="task__title completed">${obj.taskTitle}</h3>
     ...
     <div class="icons">
      ...
      `;
        this._taskesBox.insertAdjacentHTML("afterbegin", html);
    }
}
exports.default = new TaskBox();

},{"./mainFn.js":"eW3HQ","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","../helper.js":"lVRAz"}],"eW3HQ":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _iconsSvg = require("url:../../img/icons.svg");
var _iconsSvgDefault = parcelHelpers.interopDefault(_iconsSvg);
class resubleMethods {
    renderSpinner() {
        const markup = `
          <div class="spinner">
              <svg>
...
exports.default = resubleMethods;
...
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "randomCorrectNum", ()=>randomCorrectNum);
parcelHelpers.export(exports, "TIME_SEC", ()=>TIME_SEC);
const randomCorrectNum = function(min, max) {
    min = Math.ceil(min);
    max = Math.floor(max);
...
parcelHelpers.export(exports, "pushTask", ()=>pushTask);
parcelHelpers.export(exports, "taskObj", ()=>taskObj);
parcelHelpers.export(exports, "removeCompletedTask", ()=>removeCompletedTask);
const state = {
    task: {},
    taskes: []
};
const pushTask = function(taskObj) {
    const taskDate = new Date();
    const time = new Intl.DateTimeFormat("en-US", {
        timeStyle: "short"
    }).format(taskDate);
    const date = new Intl.DateTimeFormat("en-US").format(taskDate);
    state.task = taskObj;
    state.task.date = date;
    state.task.time = time;
    state.task.timeStamp = taskDate.getTime();
    state.taskes.push(state.task);
    persistTaskes();
};
const taskObj = function() {
    return state.task;
};
const removeCompletedTask = function(timeStamp) {
    const obj = state.taskes.findIndex((obj)=>obj.timeStamp === timeStamp);
    state.taskes.splice(obj, 1);
    localStorage.clear();
    persistTaskes();
};
const persistTaskes = function() {
    localStorage.setItem("taskes", JSON.stringify(state.taskes));
};
const init = function() {
    const storage = localStorage.getItem("taskes");
    if (storage) state.taskes = JSON.parse(storage);
};
init();

},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["f0HGD","aenu9"], "aenu9", "parcelRequire5c68")
